...
4. [북마크](#북마크)
5. [분리된 버튼 창](#분리된-버튼-창)
6. [번역 오버레이 모드 (HYTrans + MekiOverlayer)](#번역-오버레이-모드)
7. [설정](#설정)
8. [커맨드라인 사용법](#커맨드라인-사용법)
9. [주의사항](#주의사항)
...
1. `임시 영역을 확정 영역으로 덮어쓰기` 버튼을 누릅니다.
...
메인 화면 상단의 상태 표시줄에서 현재 **임시 영역**과 **확정 영역**의 좌표를 확인할 수 있습니다.

| 버튼 | 설명 |
|---|---|
| `임시 영역 선택` | 화면에서 드래그로 새 영역을 선택합니다. 이전 임시 영역이 있으면 그 위치에서 시작합니다. |
| `임시 영역을 확정 영역으로 덮어쓰기` | 임시 영역을 실제 인식에 사용할 확정 영역으로 등록합니다. |
| `영역 보기` | 현재 임시 영역(파란색)과 확정 영역(빨간색)을 화면에 겹쳐서 표시합니다. `Esc`로 닫습니다. |
...
> 불러온 뒤 영역을 미세 조정하려면 `임시 영역 선택`으로 다시 조정한 뒤 `임시 영역을 확정 영역으로 덮어쓰기`를 누르세요.

---

## 분리된 버튼 창

`'인식 후 복사' 버튼 분리하기` 버튼을 누르면 `인식 후 복사` 버튼만 있는 작은 별도 창이 열립니다.

- 메인 창을 닫거나 다른 곳에 두고, 이 작은 창만 화면 한쪽에 배치해 사용하기 편합니다.
...
2. `저장`을 누릅니다. 메인 화면에 `HYTrans 서버 실행`과 `MekiOverlayer 실행` 버튼이 나타납니다.
...
설정 창 하단의 `HYTrans, MekiOverlayer 연결 상태 확인` 버튼으로 두 서비스가 정상 동작하는지 테스트할 수 있습니다.
...
`설정` 버튼을 눌러 설정 창을 엽니다. 설정은 `settings.cfg` 파일에 저장됩니다.
